...
window.bufferMonitor = [];
window.bitrateMonitor = [];
window.bitrateLevelMonitor = [];
...
app.controller('DashController', function($scope, sources, contributors) {
    ...
    $scope.uorsSelected = false;
    $scope.elephantaAutoSelected = true;
    ////////////////////////////////////////
    //
    // Player Setup
    //
    ...
    $scope.toggleUorsABR = function () {
        $scope.player.setUorsABREnabled($scope.uorsSelected);
        if($scope.uorsSelected) {
            document.getElementById("uorsCons").style.display = "";
        } else {
            document.getElementById("uorsCons").style.display = "none";
        }
    }

    $scope.toggleElephantaAutoABR = function() {
        $scope.player.setElephantaAutoEnabled($scope.elephantaAutoSelected)
    }

    $scope.boChanged = function(){
        var temp = $scope.bo;
        $scope.player.setConstraintsBO(temp);
    }

    $scope.bsChanged = function(){
        var temp = $scope.bs;
        $scope.player.setConstraintsBS(temp);
    }

    $scope.rbChanged = function(){
        var temp = $scope.rb;
        $scope.player.setConstraintsRB(temp);
    }
    ...
    function updateMetrics(type) {
        ...
        if (metrics && dashMetrics && $scope.streamInfo) {
            ...
            if(type === 'video') {
                bufferMonitor.push(bufferLevel);
                bitrateMonitor.push(bitrate);
                bitrateLevelMonitor.push(index);
                //console.log("UORS Monitor: buffer level:" + bufferLevel);
            }
